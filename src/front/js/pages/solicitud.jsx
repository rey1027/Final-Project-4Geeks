...
export const Citas = () => {
  ...
  const [msjEspecialidad, setMsjEspecialidad] = useState(
    "-- Selecione una especialidad primero --"
  );
  const [msjTratamiento, setMsjTratamiento] = useState(
    "-- Selecione un especialista primero --"
  );
  ...
  useEffect(() => {
    const fetchEspecialidades = async () => {
      try {
        const response = await actions.fetchPromise(
          "/api/especialidades",
          "GET"
        );
  ...
  const handleEspecialidadChange = async (event) => {
    ...
    if (especialidad != "") {
      let data = await actions.fetchPromise(
        `/api/especialistas/especialidad/${especialidad}`,
        "GET"
      );
  ...
  const handleEspecialistaChange = async (event) => {
    ...
    if (especialista != "") {
      let data = await actions.fetchPromise(
        `/api/tratamientos/especialista/${especialista}`,
        "GET"
      );
  ...
  const handleSubmit = async (event) => {
    ...
    let response = await actions.fetchPromise("/api/citas", "POST", obj);
